 ...
 * - Medição precisa de tempo de execução usando clock()
 ...
 *
 * Métricas analisadas:
 * - Tempo de execução (em segundos com precisão de microssegundos)
...
/* ================================================================
 * SISTEMA DE MEDIÇÃO DE TEMPO E PERFORMANCE
 * ================================================================ */

/**
 * @brief Mede tempo de execução de algoritmos de ordenação padrão
 *
 * Utiliza a função clock() da biblioteca padrão C para medir com precisão
 * o tempo de CPU consumido por um algoritmo de ordenação. A medição
 * inclui apenas o tempo efetivo de processamento, excluindo I/O.
 *
 * Precisão da medição:
 * - Resolução: microssegundos (dependente do sistema)
 * - Tipo: tempo de CPU (não tempo real/wall clock)
 * - Overhead: mínimo, apenas duas chamadas de clock()
 *
 ...
 * @return Tempo de execução em segundos (precisão de ponto flutuante)
 */
double medir_tempo_ordenacao(void (*sort_fn)(void*, int, size_t, CompareFn),
                            void *arr, int n, size_t elem_size, CompareFn cmp) {
    // Marca do tempo inicial (antes da execução)
    clock_t inicio = clock();

    // Execução do algoritmo de ordenação
    sort_fn(arr, n, elem_size, cmp);

    // Marca do tempo final (após a execução)
    clock_t fim = clock();

    // Cálculo do tempo decorrido e conversão para segundos
    return ((double)(fim - inicio)) / CLOCKS_PER_SEC;
}

/**
 * @brief Mede tempo de execução específico do Quick Sort
 *
 * Versão especializada da medição de tempo para o Quick Sort, que
 * requer parâmetros diferentes (início e fim) devido à sua natureza
 * recursiva e interface de particionamento.
 *
...
void executar_todos_algoritmos(void *dados, int tamanho, size_t elem_size, CompareFn cmp,
    ...
    for (int i = 0; i < NUM_ALGORITMOS; i++) {
        ...
        if (algoritmos[i].eh_quick) {
            tempo = medir_tempo_quick_sort(algoritmos[i].quick_sort_fn, dados_copia, tamanho, elem_size, cmp);
        } else {
            tempo = medir_tempo_ordenacao(algoritmos[i].sort_fn, dados_copia, tamanho, elem_size, cmp);
        }

        // Armazena resultado com contadores
        strcpy(resultados[i].algoritmo, algoritmos[i].nome);
        resultados[i].tempo_execucao = tempo;
        resultados[i].tamanho_dados = tamanho;
        strcpy(resultados[i].tipo_dados, tipo_dados);
        resultados[i].comparacoes = contador_comparacoes;
        resultados[i].trocas = contador_trocas;
